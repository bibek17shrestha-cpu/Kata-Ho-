...
nav.innerHTML = `<a class="brand" href="${brandHref}">कता हो?</a> <div class="spacer"></div> ${user ?`<span class="user-pill">${esc(user.name)} · ${user.role === ‘rider’ ? ‘driver’ : ‘passenger’}</span><button id="logoutBtn">Log out</button>`:`<a href="/login.html">Log in</a><a href="/signup.html">Sign up</a>`} `;
...
function trailSvg() {
return `<svg viewBox="0 0 200 14" preserveAspectRatio="none"> <circle cx="4" cy="7" r="3" fill="#E8A33D"/> <line x1="4" y1="7" x2="196" y2="7" stroke="#E8A33D" stroke-width="1.3" stroke-dasharray="1 7" stroke-linecap="round" opacity="0.75"/> <circle cx="196" cy="7" r="3" fill="#3E7CB1"/> </svg>`;
}
...
el.innerHTML = msgs.map(m => `<div class="msg ${m.senderId === chatState.myId ? 'mine' : 'theirs'}">${esc(m.body)}</div>`).join(’’);
