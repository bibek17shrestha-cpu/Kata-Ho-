...
function renderRides() {
  ...
  pane.innerHTML = filtered.map(r => `
    <div class="card">
      ...
      ${r.riderContact ? `<div class="meta"><span>📞 ${esc(r.riderContact)}</span></div>` : ''}
      <div class="card-foot">
        <div class="actions">
          ${r.riderContact ? `<a class="btn-secondary" style="text-decoration:none;" href="tel:${esc(String(r.riderContact).replace(/[^\d+]/g,''))}">📞 Call</a>` : ''}
          <button class="btn-primary" data-action="chat" data-id="${r.id}" data-rider="${esc(r.riderName)}" data-from="${esc(r.from)}" data-to="${esc(r.to)}" data-contact="${esc(r.riderContact || '')}">Chat</button>
        </div>
      </div>
    </div>
  `).join('');
...
document.getElementById('browsePane').addEventListener('click', async (e) => {
  ...
  try {
    ...
    openChat(convo.id, me.id, btn.dataset.rider, btn.dataset.from, btn.dataset.to, btn.dataset.contact || null, false);
    loadInbox();
  } catch (err) {
    alert(err.message);
  }
  ...
  try {
    ...
    pane.querySelectorAll('.convo-row').forEach(row => {
      row.addEventListener('click', () => {
        openChat(row.dataset.id, me.id, row.dataset.name, row.dataset.from, row.dataset.to, row.dataset.contact || null, false);
      });
    });
  } catch (e) {
