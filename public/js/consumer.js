...
list.innerHTML = allDrivers.map(d => `<div class="card"> <div class="card-top"> <div style="display:flex; align-items:center; gap:10px;"> ${avatarHtml(d.name, 'rider')} <span class="rider-name">${esc(d.name)}</span> </div> <span class="status ${d.isAvailable ? 'avail' : 'closed'}">${d.isAvailable ? '● available' : '○ not available'}</span> </div> ${d.contact ?`<div class="contact-line"><a href="tel:${esc(d.contact)}">${esc(d.contact)}</a></div>`: ''} ${d.vehicleInfo ?`<div class="note">${esc(d.vehicleInfo)}</div>`: ''} <div class="card-foot"> <div class="actions"> <button class="btn-primary" data-action="request-driver" data-id="${d.id}" data-name="${esc(d.name)}" ${d.isAvailable ? '' : 'disabled'}>${d.isAvailable ? 'Request this driver' : 'Currently unavailable'}</button> </div> </div> </div>`).join(’’);
}
...
pane.innerHTML = myRequests.map(r => `<div class="card"> <div class="card-top"> <span class="rider-name">${r.driverId ? 'Sent to a specific driver' : 'Open to any driver'}</span> <span class="status ${r.status === 'accepted' ? 'avail' : r.status === 'declined' || r.status === 'cancelled' ? 'closed' : ''}">${r.status}</span> </div> <div class="route"> <div class="place">${esc(r.from)}</div> <div class="line">${trailSvg()}</div> <div class="place">${esc(r.to)}</div> </div> ${r.note ?`<div class="note">${esc(r.note)}</div>`: ''} <div class="card-foot"> <div class="actions"> ${r.status === 'accepted' ?`<button class="btn-primary" data-action="chat-request" data-id="${r.id}">Open chat</button>`: ''} ${r.status === 'pending' ?`<button class="btn-danger" data-action="cancel" data-id="${r.id}">Cancel</button>`: ''} <button class="btn-secondary" data-action="clear" data-id="${r.id}">Delete</button> </div> </div> </div>`).join(’’);
}
...
const warning = req && req.status === ‘accepted’
? ‘Delete this request? This will also delete your chat with the driver.’
: ‘Delete this request?’;
...
const data = await res.json();
throw new Error(data.error || ‘Could not delete this request.’);
}
...
pane.innerHTML = allRides.map(r => `<div class="card"> <div class="card-top"> <span class="rider-name">${esc(r.riderName)}</span> <span class="status">${r.seats} seat${r.seats == 1 ? '' : 's'} free</span> </div> <div class="route"> <div class="place">${esc(r.from)}</div> <div class="line">${trailSvg()}</div> <div class="place">${esc(r.to)}</div> </div> <div class="meta"><span>${fmtDate(r.date, r.time)}</span></div> ${r.note ?`<div class="note">${esc(r.note)}</div>`: ''} <div class="card-foot"> <div class="actions"> <button class="btn-primary" data-action="chat" data-id="${r.id}" data-rider="${esc(r.riderName)}" data-from="${esc(r.from)}" data-to="${esc(r.to)}">Chat</button> </div> </div> </div>`).join(’’);
}
...
pane.innerHTML = myConvos.map(c => `<div class="convo-row" data-id="${c.id}" data-name="${esc(c.otherName)}" data-from="${esc(c.rideFrom)}" data-to="${esc(c.rideTo)}" data-contact="${esc(c.otherContact || '')}" data-role="${esc(c.otherRole || '')}" data-gender="${esc(c.otherGender || '')}"> ${avatarHtml(c.otherName, c.otherRole, null, c.otherGender)} <div style="flex:1; min-width:0;"> <div class="convo-route-main">${esc(c.rideFrom)} <span class="route-arrow">→</span> ${esc(c.rideTo)}</div> <div class="convo-name">${esc(c.otherName)} <span class="role-tag ${c.otherRole === 'rider' ? 'role-driver' : 'role-passenger'}">${roleLabel(c.otherRole)}</span></div> </div> ${c.unreadCount > 0 ?`<span class="unread-badge">${c.unreadCount > 9 ? ‘9+’ : c.unreadCount}</span>`: ''} <button class="convo-clear-btn" data-clear-id="${c.id}" title="Clear this chat" aria-label="Clear this chat">✕</button> </div>`).join(’’);
