...
setTimeout(() => { btn.textContent = ‘Save vehicle info’; }, 1500);
} catch (e) {
...
pane.innerHTML = incomingRequests.map(r => `<div class="card"> <div class="card-top"> <span class="rider-name">${esc(r.consumerName)}</span> <span class="status">requested</span> </div> <div class="route"> <div class="place">${esc(r.from)}</div> <div class="line">${trailSvg()}</div> <div class="place">${esc(r.to)}</div> </div> ${r.note ?`<div class="note">${esc(r.note)}</div>`: ''} <div class="card-foot"> <div class="actions"> <button class="btn-primary" data-action="accept" data-id="${r.id}">Accept</button> <button class="btn-danger" data-action="decline" data-id="${r.id}">Decline</button> </div> </div> </div>`).join(’’);
}
...
// — fixed route listings (unchanged behavior) —
...
pane.innerHTML = myRides.map(r => `<div class="card"> <div class="card-top"> <span class="rider-name">${r.seats} seat${r.seats == 1 ? '' : 's'} free</span> <span class="status ${r.status === 'closed' ? 'closed' : ''}">${r.status === 'closed' ? '● closed' : '○ open'}</span> </div> <div class="route"> <div class="place">${esc(r.from)}</div> <div class="line">${trailSvg()}</div> <div class="place">${esc(r.to)}</div> </div> <div class="meta"><span>${fmtDate(r.date, r.time)}</span></div> ${r.note ?`<div class="note">${esc(r.note)}</div>`: ''} <div class="card-foot"> <div class="actions"> <button class="btn-secondary" data-action="toggle" data-id="${r.id}">${r.status === 'closed' ? 'Reopen' : 'Mark closed'}</button> <button class="btn-danger" data-action="delete" data-id="${r.id}">Delete</button> </div> </div> </div>`).join(’’);
}
...
pane.innerHTML = myConvos.map(c => `<div class="convo-row" data-id="${c.id}" data-name="${esc(c.otherName)}" data-from="${esc(c.rideFrom)}" data-to="${esc(c.rideTo)}" data-contact="${esc(c.otherContact || '')}"> <div> <div class="convo-name">${esc(c.otherName)}</div> <div class="convo-route">${esc(c.rideFrom)} → ${esc(c.rideTo)}</div> </div> <span class="btn-secondary">Open chat</span> </div>`).join(’’);
