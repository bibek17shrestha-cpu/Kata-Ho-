...
function renderNav(user, activeBrandHref) {
  ...
  nav.innerHTML = `
    <a class="brand" href="${brandHref}">Saathi Sawaari</a>
    <div class="spacer"></div>
    ${user
      ? `<span class="user-pill">${esc(user.name)} · ${user.role}</span><button id="logoutBtn">Log out</button>`
...
async function openChat(conversationId, myId, withName, rideFrom, rideTo) {
  ensureChatModal();
