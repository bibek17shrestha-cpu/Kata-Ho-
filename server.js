...
try {
const { rows } = await pool.query(`SELECT u.id, u.name, u.contact, dp.vehicle_info, dp.is_available FROM users u LEFT JOIN driver_profiles dp ON dp.user_id = u.id WHERE u.role = 'rider' ORDER BY dp.is_available DESC NULLS LAST, u.name ASC`);
res.json(rows.map(r => ({
id: r.id,
name: r.name,
contact: r.contact,
...
// Public, real (not fabricated) counts for the homepage stats section.
app.get(’/api/stats’, async (req, res, next) => {
try {
const [drivers, available, completed, riders] = await Promise.all([
pool.query(`SELECT COUNT(*)::int AS n FROM users WHERE role = 'rider'`),
pool.query(`SELECT COUNT(*)::int AS n FROM driver_profiles WHERE is_available = true`),
pool.query(`SELECT COUNT(*)::int AS n FROM ride_requests WHERE status = 'completed'`),
pool.query(`SELECT COUNT(*)::int AS n FROM users WHERE role = 'consumer'`)
]);
res.json({
totalDrivers: drivers.rows[0].n,
availableNow: available.rows[0].n,
completedRides: completed.rows[0].n,
totalPassengers: riders.rows[0].n
});
} catch (err) { next(err); }
});
...
if (req.user.role !== ‘rider’) return res.status(403).json({ error: ‘Riders only.’ });
const { rows } = await pool.query(
`SELECT * FROM ride_requests WHERE status = 'pending' AND (driver_id = $1 OR driver_id IS NULL) ORDER BY created_at DESC`,
[req.user.id]
);
res.json(rows.map(requestRow));
} catch (err) { next(err); }
...
try {
const { rows } = await pool.query(
`SELECT * FROM conversations WHERE (rider_id = $1 AND archived_by_rider = false) OR (consumer_id = $1 AND archived_by_consumer = false) ORDER BY created_at DESC`,
[req.user.id]
);
...
const now = Date.now();
await pool.query(
`INSERT INTO conversation_reads (conversation_id, user_id, last_read_at) VALUES ($1, $2, $3) ON CONFLICT (conversation_id, user_id) DO UPDATE SET last_read_at = $3`,
...
await pool.query(
`INSERT INTO push_subscriptions (id, user_id, endpoint, p256dh, auth, created_at) VALUES ($1, $2, $3, $4, $5, $6) ON CONFLICT (endpoint) DO UPDATE SET user_id = $2, p256dh = $4, auth = $5`,
